...
import { cookies } from 'next/headers';
import { ClientPage } from '../../../components/admin/ClientPage';
import QuestionDetail from '../../../components/admin/QuestionDetail'; // Import the new component
...
export default async function ManageContentPage({ params }: { params: { slug?: string[] } }) {
    ...
    try {
        const currentLevelIndex = slug.length;
        // FIX #1: The logic now allows for a slug length of 5 (for a single question)
        if (currentLevelIndex > levels.length) throw new Error("Invalid content path.");

        // --- BREADCRUMB FIX #2: Always start with a home/dashboard link ---
        const breadcrumbs = [{ name: 'Dashboard', href: '/admin/manage' }];
        
        // Fetch data for each part of the slug to build the rest of the breadcrumbs
        const breadcrumbDataPromises = slug.map((id, index) => {
            // Stop at the topic level for breadcrumbs, as a question doesn't have a 'name'
            if (index >= levels.length) return null; 
            const levelForBreadcrumb = levels[index];
            return fetchData(`${API_BASE_URL}${levelForBreadcrumb.singleEndpoint}/${id}`);
        });
        const breadcrumbItems = (await Promise.all(breadcrumbDataPromises)).filter(Boolean);
        ...
        breadcrumbItems.forEach((item, index) => {
            breadcrumbs.push({
                name: item.name || item.chapterName || item.title || 'Detail',
                href: `/admin/manage/${slug.slice(0, index + 1).join('/')}`
            });
        });

        // --- SINGLE QUESTION VIEW LOGIC ---
        // If the slug length is 5, we are viewing a single question.
        if (currentLevelIndex === 5) {
            const questionId = slug[4];
            const question = await fetchData(`${API_BASE_URL}/questions/${questionId}`); // Assuming this endpoint exists
            if (!question) throw new Error("Question not found.");
            
            // Add a final breadcrumb for the detail view
            breadcrumbs[breadcrumbs.length -1].name = "Question Detail";

            // Render the new detail component
            return (
                <div className="bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8">
                    <QuestionDetail question={question} breadcrumbs={breadcrumbs} />
                </div>
            );
        }

        // --- LIST VIEW LOGIC (Unchanged for other levels) ---
        const currentLevel = levels[currentLevelIndex];
        const parentId = currentLevelIndex > 0 ? slug[currentLevelIndex - 1] : '';
        const listUrl = currentLevelIndex > 0
            ? `${API_BASE_URL}${currentLevel.listEndpoint}/${parentId}`
